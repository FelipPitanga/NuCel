import base64,hashlib,hmac,importlib.util,io,json,threading,time,unittest,urllib.request,urllib.error
...
class ConnectorTests(unittest.TestCase):
 ...
 def setUpClass(cls):
  ...
  cls.calls=[]
  im=Image.new('RGB',(1080,2400),'purple');out=io.BytesIO();im.save(out,'PNG');cls.png=out.getvalue()
  def fake(*args):
   cls.calls.append(args)
   if args==('devices',):return b'List of devices attached\nALLOWED\tdevice\nHIDDEN\tdevice\n'
   if 'screencap' in args:return cls.png
   return b''
  a.adb=fake
 ...
 def test_denied_serial(self):self.assertEqual(self.req('/frame?serial=HIDDEN')[0],403)
 ...
 def test_frame_dimensions(self):
  code,body=self.req('/frame?serial=ALLOWED');self.assertEqual(code,200)
  with Image.open(io.BytesIO(body)) as im:self.assertEqual(im.size,(450,1000))
 def test_tap_maps_full_resolution(self):
  code,_=self.req('/action',{'serial':'ALLOWED','type':'tap','x':.5,'y':.5});self.assertEqual(code,200);self.assertIn(('-s','ALLOWED','shell','input','tap','540','1200'),self.calls)
 ...
 def test_native_key(self):
  ...
  self.assertIn(('-s','ALLOWED','shell','input','keyevent','3'),self.calls)
